...
export default async function UnconfirmedBookingsPage() {
  try {
    ...
    if (authError || !user) {
      console.error("Unconfirmed Bookings: No user found", {
        error: authError?.message,
        hasUser: !!user,
        timestamp: new Date().toISOString(),
      });
      redirect("/therapist-login?redirect=/therapist-dashboard/bookings/unconfirmed");
    }
    ...
    if (userError || !dbUser) {
      console.error("Unconfirmed Bookings: User not found in database", {
    ...
    if (dbUser.role !== "THERAPIST") {
      console.error("Unconfirmed Bookings: User is not therapist", {
        role: dbUser.role,
        userId: user.id,
        timestamp: new Date().toISOString(),
      });
    ...
    if (dbUser.account_status !== "ACTIVE") {
      console.error("Unconfirmed Bookings: Account not active", {
    ...
    // 3. Fetch unconfirmed bookings (PENDING status)
    ...
    return <BookingsPageClient bookings={bookings} type="unconfirmed" />;
  } catch (error) {
    console.error("Unconfirmed Bookings: Server error", error);
    redirect("/therapist-login?redirect=/therapist-dashboard/bookings/unconfirmed");
  }
}
