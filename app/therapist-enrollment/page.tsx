...
export default function TherapistEnrollmentPage() {
        ...
        if (session?.user) {
          ...
          try {
            ...
            if (roleCheckResponse.ok) {
              ...
              if (enrollmentData.enrolled && enrollmentData.role === "THERAPIST") {
                // User is already a therapist - redirect to dashboard
                console.log("User is already enrolled as THERAPIST, redirecting to dashboard");
          ...
          // User doesn't have THERAPIST role - proceed with enrollment form
  ...
  const checkEmailExists = async (emailToCheck: string): Promise<boolean> => {
    ...
    try {
      const response = await fetch("/api/therapists/check-email", {
        method: "POST",
        headers: {
  ...
  const handleGoogle = async () => {
    ...
    try {
      ...
      const callbackUrl = `${currentOrigin}/auth/callback?source=therapist-enrollment`;
      
      console.log('🔐 Therapist Enrollment OAuth redirect:', callbackUrl);
    ...
    <div className="relative min-h-screen bg-[#0b0b0b] text-white overflow-hidden flex items-center justify-center px-4 sm:px-6 md:px-8 py-10 md:py-12">
      ...
      <div className="relative z-10 w-full max-w-5xl xl:max-w-6xl grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
        ...
        <div className="lg:col-span-1 space-y-4 bg-white/5 border border-white/10 rounded-2xl p-5 md:p-6 backdrop-blur-sm">
          ...
          <div className="text-xs md:text-sm text-white/60">Therapist enrollment</div>
